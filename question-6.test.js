 ...
 */
describe('partition', () => {
  it('It should return empty arr', () => {
    expect(partition(['car', 'train'])).toEqual([]);
  });
  it('It should return empty arr', () => {
    expect(partition('test', (a) => a)).toEqual([]);
  });
  it('It should not be empty arr', () => {
    const vehicles = ['car', 'plane', 'train'];
    expect(partition(vehicles, (vehicle) => vehicle === 'train')).not.toBe([]);
  });
  it('Return true and false arrays', () => {
    const vehicles = ['car', 'plane', 'train'];
    const filtered = partition(vehicles, (vehicle) => vehicle === 'train');
    expect(filtered).toHaveLength(2);
  });
  it('It should return false arr empty', () => {
    const vehicles = ['train', 'train', 'train'];
    const filtered = partition(vehicles, (vehicle) => vehicle === 'train');
    expect(filtered[1]).toEqual([]);
  });
  it('It should return true arr empty', () => {
    const vehicles = ['train', 'train', 'train'];
    const filtered = partition(vehicles, (vehicle) => vehicle === 'bus');
