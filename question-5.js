...
// Get Delivery Status from this function
const getDeliveryStatus = (delivery) => {
  const { isDelivered, isReturned } = delivery;
  if (!isDelivered && !isReturned) {
    return DELIVERY_STATUS.PENDING;
  } else if (isDelivered && !isReturned) {
    return DELIVERY_STATUS.DELIVERED;
  } else if ((!isDelivered && isReturned) || (isDelivered && isReturned)) {
    return DELIVERY_STATUS.RETURNED;
  }
};
const fixDate = (date) => date.getDate(); //Fix dates with value

//Get Pending Deliveries with giving date
const getDeliveries = (arr, date) => {
  return arr.filter(
    (delivery) => delivery.plannedDeliveryDate.getDate() === date,
  );
};

const getDashboardInformation = (deliveries, today, tomorrow) => {
  const todayDate = fixDate(today);
  const tomorrowDate = fixDate(tomorrow);

  //Get delivery status if pending or not
  const pendingDeliveries = deliveries.filter(
    (delivery) => getDeliveryStatus(delivery) === DELIVERY_STATUS.PENDING,
  );

  const todaysDeliveries = getDeliveries(pendingDeliveries, todayDate);
  const tomorrowsDeliveries = getDeliveries(pendingDeliveries, tomorrowDate);

  let deliveryList = {};
  deliveryList['today'] = todaysDeliveries;
  deliveryList['tomorrow'] = tomorrowsDeliveries;
  return deliveryList;
};
